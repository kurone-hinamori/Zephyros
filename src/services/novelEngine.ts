...
8. **文章崩れ・フレーズ連続反復・読点異常・孤立カタカナ壊れ文のチェック**:
   - 同一文節の無限繰り返しや読点（、）の過剰多用が含まれる場合は即座に hasCriticalError: true とし、"type": "contradiction", "comment": "文章の同一フレーズ無限ループまたは読点過剰崩れを検出" と指定してください。
   - 「アしなければならない」「カする」のように一文字カタカナ＋動詞語尾による文脈壊れ・トークン欠損文が含まれる場合も即座に hasCriticalError: true とし、"type": "contradiction", "comment": "一文字カタカナ＋動詞語尾（例: アしなければならない）のトークン欠損・文法崩れを検出。単に一文字を削除するのではなく文脈を推定して完成された日本語表現への再構築が必要です。" と指定してください。
...
10. **孤立カタカナ・壊れた動詞フレーズの文脈推定修復**: 「アしなければならない」「カする」のような一文字カタカナ＋動詞語尾の壊れた文が含まれる場合、単に一文字を削除するのではなく、直前の文脈（キャラクターの行動・台詞・感情など）から本来の意図を正確に推測し、「声を発し、応答しなければならない」「動かなければならない」のように自然で重厚な完成された日本語文章へ再構築・加筆修正してください。
11. 解説や挨拶、思考プロセス(<think>)は一切含めず、純粋な修正本文のみを出力してください。`,
...
上記本文から、人物・品物・地名・用語・ルビの新規登場および設定の変化を抽出し、JSON形式で返してください。`;
  ...
  public static cleanForeignNoiseText(text: string): string {
    ...
    // 6.9 1〜3文字カタカナ片 ＋ （日本語注釈） 但し書きノイズ（例: "カ（不器用）" → "不器用"）の自動純化
    clean = clean.replace(/([ァ-ヴー・]{1,3})\s*[\(（]([一-龠ぁ-んァ-ヴー・]{2,})[\)）]/g, '$2');
