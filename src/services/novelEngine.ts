...
2. **日本語原則および外国語・ノイズ単語の絶対禁止**: 本文は100%日本語で記述してください。地名や作品固有コード等を除き、脈絡のない英単語（例: get, gett, oversized）、壊れたカッコ単語（例: get（get/gett）、（むget））、異言語文字（ハングル・他国語等）を地の文や会話文に混入させることは固く禁止します。
...
3. **日本語原則および外国語・ノイズ単語の絶対禁止**: 本文は100%日本語で記述してください。地名や作品固有コード等を除き、脈絡のない英単語（例: get, gett, oversized）、壊れたカッコ単語（例: get（get/gett）、（むget））、異言語文字（ハングル・他国語等）を地の文や会話文に混入させることは固く禁止します。
...
上記データに対する校閲・誤読修正を行い、修正後のJSONを出力してください。`;
  ...
  static resolveSystemPrompt(
    ...
    if ((key === 'writeSceneContent' || key === 'rewriteSceneWithFeedback') && !prompt.includes('ノイズ単語')) {
      prompt += '\n\n【日本語原則・外国語翻訳制約】脈絡のない英単語（get, gett等）、壊れたカッコ単語（（get/gett）等）、異言語文字（ハングル等）の混入は絶対禁止です。外国人キャラクターが本国語を話す場合は必ず直後に『（日本語訳）』をカッコ書きで併記してください（例: 「Spasibo（ありがとう）」）。';
    }

    if (key === 'proofreadScene' && !prompt.includes('ノイズ外国語')) {
      prompt += '\n\n【ノイズ単語・外国語訳の厳格検出指示】地の文や会話文に不自然に混ざる英単語（get, gett, oversized等）、壊れたカッコ表現（get（get/gett）、（むget）等）、異言語文字（ハングル等）は必ず typo として検出し originalText と suggestedText を指定してください。また、外国人キャラの本国語台詞に日本語訳のカッコ書きがない場合も typo として指摘してください。';
    }
